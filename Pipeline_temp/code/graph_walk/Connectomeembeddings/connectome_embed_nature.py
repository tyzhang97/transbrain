...
def create_embedding(dir_name, input_edge_list, output_embedding, current_dti, current_name,
                     permutation_no=1000, lesion_node = 0, dimensions=40, walk_length=40,
                     num_walks=800, window_size=2, iter=1, workers=10, p=0.01,q=0.1, sg=0,
    ...
    with open(os.path.join(dir_name,current_name + '_walks_lesion_' + str(lesion_node)), 'wb') as f:
...
def learn_embeddings(walks, dimensions, window_size, workers, iter, output_embedding, sg=0):
    ...
    for i in walks:
        ...
        input_walks.append(element)

    with open('./walks.pkl','wb') as f:
        pickle.dump(input_walks,f)
    f.close()

    model = Word2Vec(input_walks, vector_size=dimensions, window=window_size, min_count=0, sg=sg,
                     workers=workers, epochs=iter) 
...
def normalize_embeddings(word2Vecmodel):
    ...
    word2Vecmodelsorted = np.zeros([word2Vecmodel.wv.vectors.shape[0], word2Vecmodel.wv.vectors.shape[1]])
    for i in range(word2Vecmodel.wv.vectors.shape[0]): 
        word2Vec_init_vector=word2Vecmodel.wv[str(i)].reshape(1,-1).copy() 
        word2Vecmodelsorted[i] = normalizer.fit_transform(word2Vec_init_vector)
    return word2Vecmodelsorted
